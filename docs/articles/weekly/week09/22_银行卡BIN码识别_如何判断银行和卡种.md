...
```
622848   040256489001   8
└──┬───┘ └─────┬──────┘ └───── 校验位（Luhn）
   │           └─────────────── 银行内部账号（可变长度，第 7-18 位）
   └─────────────────────────── BIN（前 6 位 = 农行金穗借记卡）
```

注意：**MII 是 BIN 的第 1 位**，而不是独立于 BIN 之外的一段。`622848` 这 6 位整体由银联分配给农行，识别时直接取卡号前 6 位查表即可，不必单独解析 MII（本例首字符 `6` 是 MII，表示金融业 / 银联，已包含在 BIN 内）。
...
public class BankCardInfoUtil {
    ...
    public static void main(String[] args) {
        ...
        // 合法但 BIN 未收录（622700 开头，表中无此段）
        System.out.println(parse("622700888888888882")); // null（Luhn 通过，BIN 表没有）
        // Luhn 校验失败（末位校验位数字 8 误写成 9）
...
### 5. 测试卡号与真实卡号
`4012888888881881`（Visa 官方测试卡）、`5555555555554444`（MasterCard 官方测试卡）、`4532015112830366`（常见支付网关测试号）等是公开测试卡号，Luhn 合法但没有真实银行账户。别把它们当"有效卡"放行支付，BIN 识别应加"测试卡号"黑名单。
