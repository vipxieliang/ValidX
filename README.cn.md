...
如果你用过 PHP 的 ThinkPHP 框架，你会发现它内置了非常丰富的验证规则：`mobile`、`idcard`、`zip`、`alphaNum` 等等，开箱即用，简单方便。
...
**为什么 Java 不能像 ThinkPHP 那样简单？** 这就是 ValidX 诞生的初衷。
...
基于这些痛点，我们创建了 ValidX，目标是：**让 Java 验证像 ThinkPHP 一样简单好用，同时更强大**

1. **提供 90+ 中国场景验证器** - 从身份证到快递单号，从 QQ 号到车牌号，覆盖中国业务的方方面面，比 ThinkPHP 更全面
2. **两种使用方式** - 注解式（适合 DTO 对象验证）和链式 API（适合动态验证），比 ThinkPHP 更灵活
