...
| **手机相关验证** | [@IMSI](#imsi) | IMSI国际移动用户识别码验证 | 1.2.1 | - |
| **手机相关验证** | [@ICCID](#iccid) | ICCID集成电路卡识别码验证 | 1.2.1 | - |
...
#### @IMSI
* 校验规则：IMSI验证，验证字符串是否为有效的国际移动用户识别码（ITU-T E.212 / 3GPP TS 23.003）。IMSI 标识"用户/SIM 卡"（IMEI 标识"设备"），结构为 MCC(3位) + MNC(2~3位) + MSIN(≤10位)，中国大陆 MCC 为 460。去除空格与连字符后应为 14~15 位纯数字；IMSI 无内置校验位，真实性需运营商网络侧确认。
* 示例格式：`460001234567890`、`4600-0123-4567-890`
* 使用示例：
  ```java
  // 注解方式使用
  @IMSI
...
#### @ICCID
* 校验规则：ICCID验证，验证字符串是否为有效的集成电路卡识别码（ITU-T E.118 / GSMA SGP.22）。ICCID 印刷在 SIM/eSIM 卡体上，唯一标识"卡"本身（IMSI 标识"签约用户/SIM"、IMEI 标识"设备"）。现代 UICC/eSIM 的 ICCID 为 20 位数字，结构为 89(电信行业标识) + 国家码(ISO 3166-1，中国 86) + 运营商代码 + 用户账号 + Luhn 校验位，中国大陆 SIM 卡通常以 `8986` 开头。去除空格与连字符后应为 20 位纯数字，且整串通过 Luhn 校验（末位为校验位，可防输入笔误）；卡的真实性仍需运营商侧确认。
* 示例格式：`89860115244939092661`、`8986-0115-2449-3909-2661`
* 使用示例：
  ```java
  // 注解方式使用
  @ICCID
