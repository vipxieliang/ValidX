...
**Current Version: v1.2.0** (In Development, To Be Released)
...
|---------|-------------|--------------|------------------|-----------|-----------------|
| **v1.2.0** | TBD | New `@StartsWithAny`, `@EndsWithAny` annotations; `@FileSize` with MIME type validation; Code refactoring for 20+ validators | Chain API parameter change: `isStartsWith()`/`isEndsWith()` from `String[]` to `String` | [View](docs/version/v1.2.0/CHANGELOG.md) | - |
