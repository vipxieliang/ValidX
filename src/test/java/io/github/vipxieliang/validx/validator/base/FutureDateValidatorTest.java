...
public class FutureDateValidatorTest {
    ...
    public void testValidFutureDate() {
        ...
        // 测试有效的未来日期时间
        String futureDateTime = LocalDateTime.now().plusDays(1).withHour(12).withMinute(0).withSecond(0)
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        assertTrue(defaultValidator.isValid(futureDateTime, null), futureDateTime + " should be valid");
    }
    ...
    public void testInvalidPastDate() {
        ...
        // 测试无效的过去日期时间
        String pastDateTime = LocalDateTime.now().minusDays(1).withHour(12).withMinute(0).withSecond(0)
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        assertFalse(defaultValidator.isValid(pastDateTime, null), pastDateTime + " should be invalid");
    }
    ...
    public void testValidTodayWithIncludeToday() {
        ...
        // 测试今天的日期时间（包含今天）
        String todayDateTime = LocalDate.now().atStartOfDay()
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        assertTrue(includeTodayValidator.isValid(todayDateTime, null), todayDateTime + " should be valid when includeToday=true");
    }
