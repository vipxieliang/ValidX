...
public class GeoPointValidationChainTest {
    ...
    public void testWithConfigurationAPI() {
        // 测试与配置API结合使用
        ValidationPlus chain1 = ValidationPlus.init()
            .config(io.github.vipxieliang.validx.chain.ValidationConfig.GLOBAL_NOT_NULL);
        chain1.isGeoPoint((Object)null);
        assertFalse(chain1.passed(), "配置NOT_NULL时null应该不通过验证");

        ValidationPlus chain2 = ValidationPlus.init()
            .config(io.github.vipxieliang.validx.chain.ValidationConfig.GLOBAL_NOT_EMPTY);
